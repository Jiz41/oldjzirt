...
## 残課題（次の一手）
- ~~HF本番(v11)への同修正の移植~~ → **完了（2026-07-05、V11.1.0としてgenerateSeitenreiBetsのみ差し替え・移植済み）**
- ~~6/9-16の素点劣化（追捕捉39→28%、微風集中）のv11側調査~~ → **解決（2026-07-05）**
  犯人はV11.0.1のgetScenarioCoeffsテーブル改訂（追winner捕捉が74.7%→70.8%、微風6/9以降は73.1%→65.7%。
  勝者係数は不変、逃・自を押し上げて追を追い落とす片道劣化・取り逃し12件/拾い返し0件）。
  実験: 候補E(旧v10テーブル全面復元)=35.4%/78.5%、候補F(追のみ復旧)=35.5%/73.6%、現行v11=35.2%/73.2%
  → 候補E採用、V11.1.1としてHF本番へデプロイ済み（engines/candidate_E.js, result_candidate_E.json）
