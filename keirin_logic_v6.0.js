// 真自在律 実質Ver8.0 - 捌風旋炉 枢（はっぷうせんろ くるる）実装
...
//  - 激突ライン共倒れ予測による「展開的必然」の穴抽出ロジック完遂,さすがにつかれた
//
// ------------------------------------------------------------------------------------
// 🗃️ 係数設定// 
// ------------------------------------------------------------------------------------
...
function generateScenarioWagers(results) { 
    ...
    // 3. 全条件合致の確認
    if (isTenunZero && windSpeed <= 3.0) {
        // ファイナルスコア上位4名（椅子取りゲーム）を参照
        ...
        const trueIchiyo = top4.find(p => {
            ...
            // 人（的中率）：33バンク差し優位性 2.06%以上
            return (p.style === '差' || p.style === '両') && (stats && stats.hit_rate >= 0.0206);
        });

        if (trueIchiyo) {
            superiorPatternMessage = `【壱耀晴乾ノ象】天命、${trueIchiyo.id}番車に収束せり。`;
        }
    }
  ...
  logMessage("[KOUTENREI] 荒天令リスクバイアス (C係数) の計算を開始..."); 
    ...
    tempPlayers.forEach(p => { 
        ...
        if (BANK_DATA.length === 333) { 
            ...
            logMessage(`[C_short] 選手ID ${p.id} に短走路(${BANK_DATA.length}m)ブーストを適用。`); 
        } 
...
if (isHighPressure && p.recent.startsWith('1')) { 
    // 風速0mなら1.0（減点なし）、4mなら0.98、8mなら0.96と動的に変化させる
    const mentalAdj = 1.0 - (v * 0.005); 
    p.final_score = p.final_score * mentalAdj;
    logMessage(`[C_mental] 選手ID ${p.id} に風速連動プレッシャー減点(${mentalAdj.toFixed(3)})を適用。`); 
}
...
// --- 修正後 ---
if (rivalAutos >= 2) { 
    // 風速0mなら1.0、4mで0.972、7mで0.951。物理的な負荷が高い時ほど、包囲網のダメージが深刻化するロジック
    const targetAdj = 1.0 - (v * 0.007);
    targetPlayer.final_score *= targetAdj; 
    logMessage(`[C_target] ターゲット選手ID ${targetPlayer.id} に風速連動包囲網リスク(${targetAdj.toFixed(3)})を適用。`); 
}
...
if (suicideRiskLineMembers.has(p.id)) {
    // 風速 7m を最大(1.0)とした適用比率を算出。無風なら 0。
    const suicideRate = Math.min(v, 7.0) / 7.0;
    // ペナルティ値（例: 0.90）への到達度を風速で制御
    const dynamicSuicidePenalty = 1.0 - ((1.0 - SUICIDE_PENALTY) * suicideRate);
    
    p.final_score *= dynamicSuicidePenalty;
    logMessage(`[C_suicide] 選手ID ${p.id} に風速連動消耗ペナルティ(${dynamicSuicidePenalty.toFixed(3)})を適用。`);
}
            else {
...
// runScenarioSimulation 関数 (シナリオ別シミュレーションの実行) - 修正版
function runScenarioSimulation(basePlayers, allSeriInfos, settings, BANK_DATA, applyKoutenrei, lineInput, windSpeed, windDirection) { 
    ...
    scenarios.forEach(scenario => { 
        ...
        scenarioPlayers.forEach(p => { 
            ...
            // 全ての補正が完了したこのタイミングで加算
            integratedScores[p.id] += p.final_score; 
            
            logMessage(`${logPrefix} 最終確定: 選手ID ${p.id} スコア ${p.final_score.toFixed(3)}`);
        }); 
...
function generateSeitenreiBets(ranking) {
    ...
    return {
        sanrentan: [[r[0], r[1], r[2]], [r[0], r[2], r[1]], [r[1], r[0], r[2]]],
        sanrenpuku: [[r[0], r[1], r[2]], [r[0], r[1], r[3] || r[2]]]
    };
}
