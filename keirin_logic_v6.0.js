...
function generateScenarioWagers(results) { 
    ...
    const r = results.map(p => p.id); // 上位順の車番配列（1回に集約）

    // --- 新・壱耀判定（椅子取りゲーム） ---
    let superiorPatternMessage = ""; 
    const top4 = results.slice(0, 4); // 上位4名（椅子）を確保
    ...
    if (trueIchiyo) {
        superiorPatternMessage = `【壱耀晴乾ノ象】特異点：${trueIchiyo.id}番車（${trueIchiyo.name}）`;
    }
       
    // 三連単: 計算順位 1位-2位-3位 / 1位-3位-2位 / 2位-1位-3位 の3点
    const tritan = [ 
        `${r[0]}-${r[1]}-${r[2]}`, 
        `${r[0]}-${r[2]}-${r[1]}`, 
        `${r[1]}-${r[0]}-${r[2]}` 
    ...
    let tri2 = ''; 
    if (r.length >= 4) { 
        tri2 = [r[0], r[1], r[3]].sort((a, b) => a - b).join('='); 
    } 
    const trifuku = [tri1, tri2].filter(t => t.length > 0).join(', '); 

    // 【重要】ichiyo（メッセージ）を戻り値に追加
