...
function generateScenarioWagers(results) { 
    // results はすでに final_score で降順ソート済み
    if (!results || results.length < 3) return { tritan: '---', trifuku: '---' };

    const r = results.map(p => p.id); // 上位順の車番配列
    ...
    // 三連複: 計算順位 1-2-3位 / 1-2-4位 の2点（ソートして美しく表示）
